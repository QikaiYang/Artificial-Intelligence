...
def evaluate_accuracies(predicted_sentences, tag_sentences):
    ...
    for pred_sentence, tag_sentence in zip(predicted_sentences, tag_sentences):
        ...
        for pred_wordtag, real_wordtag in zip(pred_sentence, tag_sentence):
            confusion_matrix[real_wordtag[1]][pred_wordtag[1]] += 1 #
    ...
    all = 0
    for tag in tagset:
        all_recall += confusion_matrix[tag][tag]
        all += sum(confusion_matrix[tag].values())
        recall_tag = confusion_matrix[tag][tag]/sum(confusion_matrix[tag].values())
        print(tag + "  recall is  " + str(recall_tag))
    print("total recall is " + str(all_recall/all*1.0))
    #------------
