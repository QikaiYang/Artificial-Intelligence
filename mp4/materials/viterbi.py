...
def viterbi(train, test):
    ...
    for sentence in test:
        ...
        for j in range(len(all_tag)):
            dp_array[0][j] = get_initial(initial_count, all_tag[j]) + get_emission(emission_count,all_tag[j],sentence[i]) 
        i += 1
        while(i<len(sentence)):
            for j in range(len(all_tag)): #next col
                dp_array[i][j] = dp_array[i-1][0] + get_transfer(transfer_count, all_tag[0], all_tag[j]) + get_emission(emission_count,all_tag[j],sentence[i])
                record_array[i][j] = 0
                for k in range(len(all_tag)): #pre col
                    cmp = dp_array[i-1][k] + get_transfer(transfer_count, all_tag[k], all_tag[j]) + get_emission(emission_count,all_tag[j],sentence[i]) 
                    if(cmp > dp_array[i][j]):
                        dp_array[i][j] = cmp
                        record_array[i][j] = k
            i += 1
